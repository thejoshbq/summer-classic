...
// Explicit rather than fs.readdirSync(SOURCE_DATA_DIR): pkg's snapshot fs
// can serve individual asset files but can't enumerate a snapshotted
// directory (throws "Pkg: Error reading from file" on every packaged launch).
const DATA_FILES = ['players.json', 'teams.json', 'standings.json', 'bracket.json', 'game.json', 'draft.json'];
...
function seedDataDirIfMissing() {
  ...
  for (const entry of DATA_FILES) {
    const src = path.join(SOURCE_DATA_DIR, entry);
    if (fs.existsSync(src)) {
      fs.writeFileSync(path.join(DATA_DIR, entry), fs.readFileSync(src));
    }
  }
}

// A failed seed shouldn't be fatal — readSync() below already falls back to
// defaults for a missing/corrupt data file, so a partially- or un-seeded
// DATA_DIR is a supported degraded state, not a crash.
try {
  seedDataDirIfMissing();
} catch (err) {
  crashLog.logWarning('seedDataDirIfMissing', err);
}
...
module.exports = { makeStore, appDataDir };
